...
![avatar](../../../Documents/tencent files/1447380573/filerecv/image-20211028003508356.png)
...
接下来是 v10 的数组，再经过下面的运算，这个运算不难理解，是矩阵的相乘与数的相加，如果逆向就反过来即可。
...
在main函数中，程序调用division函数执行依次除法，在division函数中进行判断，如果除数为0则抛出"Division by zero condition!"异常，接着catch块会捕获一个类型为字符串的异常，也就是上面除数为0时抛出的异常
